...
const ExpressiveGraph: React.FC<Props> = ({ results, submissions }) => {
  const graphRef = useRef<HTMLDivElement>(null);
  const renderIdRef = useRef<string>(
    `expressive-graph-${Math.random().toString(36).slice(2)}`
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (graphRef.current) {
          graphRef.current.innerHTML = `<div class=\"text-sm text-gray-500\">Unable to render Expressive chart</div>`;
        }
  ...
  if (results.length === 0) return <div className="text-center p-8 text-gray-500">No Expressive results yet</div>;
  return <div ref={graphRef} className="w-full min-h-96" />;
};

export default ExpressiveGraph;
