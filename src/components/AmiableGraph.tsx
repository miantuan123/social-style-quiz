...
const AmiableGraph: React.FC<Props> = ({ results, submissions }) => {
  const graphRef = useRef<HTMLDivElement>(null);
  const renderIdRef = useRef<string>(
    `amiable-graph-${Math.random().toString(36).slice(2)}`
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (graphRef.current) {
          graphRef.current.innerHTML = `<div class=\"text-sm text-gray-500\">Unable to render Amiable chart</div>`;
        }
  ...
  if (results.length === 0) return <div className="text-center p-8 text-gray-500">No Amiable results yet</div>;
  return <div ref={graphRef} className="w-full min-h-96" />;
};

export default AmiableGraph;
