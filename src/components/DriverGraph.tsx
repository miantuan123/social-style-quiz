...
interface Props {
  results: QuizResult[];
  submissions: Array<{ id: string; name: string }>; // must match results order
}

const DriverGraph: React.FC<Props> = ({ results, submissions }) => {
  const graphRef = useRef<HTMLDivElement>(null);
  const renderIdRef = useRef<string>(
    `driver-graph-${Math.random().toString(36).slice(2)}`
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (graphRef.current) {
          graphRef.current.innerHTML = `<div class="text-sm text-gray-500">Unable to render Driver chart</div>`;
        }
  ...
  if (results.length === 0) {
    return (
      <div className="text-center p-8 text-gray-500">No Driver results yet</div>
    );
  }
